...
@section('content_header')
    <form method="GET" action="{{ route('admin.models.index') }}">
        <div style="display: flex; justify-content: space-between;">
            ...
            @endcan
            @can('create-document')
                <a href="{{ route('admin.models.create') }}" class="btn bg-info">
                ...
                <tbody>
                    @forelse ($models as $model)
                        <tr>
                            ...
                            @can('edit-document')
                                <td class='px-1'>
                                    <a href="{{ Storage::url($model->document) }}" target="blank"
                                        class="btn btn-default btn-xs btn-block" title="Clique para baizar o documento.">
                            ...
                            @can('delete-document')
                                <td class="px-1">
                                    <form method="POST" onsubmit="return(confirmaExcluir())"
                                        action="{{ route('admin.models.destroy', ['id' => $model->id]) }}">
                                        @csrf
                                        @method('DELETE')
