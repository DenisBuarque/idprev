...
@section('content')
    ...
    <form method="POST" action="{{ route('admin.testimonies.update', ['id' => $testimonie->id]) }}" enctype="multipart/form-data">
        @csrf
        @method('PUT')
                ...
                <div class="row">
                    <div class="col-sm-12">
                        <div class="form-group">
                            <small>Nome:</small>
                            <input type="text" name="name" value="{{ $testimonie->name ?? old('name') }}" placeholder="Digite o título do evento"
                    ...
                    <div class="col-md-12">
                        <div class="form-group">
                            ...
                            <textarea name="description" class="form-control @error('description') is-invalid @enderror">{{ $testimonie->description ?? old('description') }}</textarea>
                            @error('description')
                                <div class="text-red">{{ $message }}</div>
                            @enderror
                        </div>
